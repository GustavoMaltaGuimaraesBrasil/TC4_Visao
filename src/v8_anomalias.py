...
Carrega rostos conhecidos de src/Entrada/conhecidos para fazer reconhecimento e grava resultados em src/Saida/v8.
"""
...
class FaceTracker:
    ...
    def __init__(self, max_distance: float = 100.0, max_frames_lost: int = 30, 
                 embedding_threshold: float = 0.91, known_faces: Optional[Dict[str, List[np.ndarray]]] = None):
        """
        ...
        embedding_threshold: threshold de similaridade (0-1) para reconhecimento facial.
                             Valores mais altos são mais restritivos (ex: 0.91 = 91% de similaridade)
...
def _load_known_faces(pasta_conhecidos: Path, embedding_threshold: float = 0.91) -> Dict[str, List[np.ndarray]]:
    """
...
def _recognize_face(embedding: np.ndarray, known_faces: Dict[str, List[np.ndarray]], 
                   threshold: float = 0.91) -> Optional[str]:
    """
...
def detectar_faces_emocoes_video(
    ...
    if caminho_saida is None:
        # Define a pasta de saída padrão (v8 agora guarda anomalias)
        pasta_saida = PROJECT_ROOT / "src" / "Saida" / "v8"
    ...
    known_faces = _load_known_faces(pasta_conhecidos, embedding_threshold=0.91)
    ...
    face_tracker = FaceTracker(
        ...
        embedding_threshold=0.91,  # Threshold de similaridade para reconhecimento facial (91%)
